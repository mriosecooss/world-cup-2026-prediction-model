...
// v2: 12-month half-life (was 18) + hard 5-year cutoff → recent form matters more.
const HALF_LIFE_MONTHS = 12;
const MAX_AGE_YEARS = 5;
const recency = (tsSec, nowSec) => {
  const ageMonths = (nowSec - tsSec) / (30.44 * 86400);
  if (ageMonths > MAX_AGE_YEARS * 12) return 0; // hard cutoff
...
console.log(`Time-decay: ${HALF_LIFE_MONTHS}-month half-life, ${MAX_AGE_YEARS}-year hard cutoff`);
...
writeFileSync(D("elo-calibrated.json"), JSON.stringify({ matchesApplied: applied, skipped, source: srcFile, halfLifeMonths: HALF_LIFE_MONTHS, ratings }, null, 2) + "\n");
